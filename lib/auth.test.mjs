...
test("falls back to documented invite codes when env vars are missing", async () => {
  ...
  try {
    ...
    assert.equal(auth.getRequiredScopeForPath("/ai-learning"), "slides");
    assert.equal(
      auth.getRequiredScopeForPath("/ai-learning/ai-learning-collection.html"),
      "slides"
    );
    assert.equal(auth.getRequiredScopeForPath("/api/learning/feynman"), "slides");
  } finally {
